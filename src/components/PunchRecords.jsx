...
const PunchRecords = () => {
  ...
  useEffect(() => {
    const fetchPunchRecords = async () => {
      try {
        const response = await axios.get('http://localhost:3000/punch-records', {
          headers: {
        ...
        // Updating state with the data from the API
  ...
  return (
    <div className="w-full max-w-xs p-2 bg-white dark:bg-black rounded-lg shadow-md h-auto flex flex-col justify-center text-xs md:text-sm absolute right-0 top-0"> {/* Positioned to the rightmost side */}
